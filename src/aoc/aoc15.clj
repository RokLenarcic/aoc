...
(defn man-dist [p1 p2] (reduce + (mapv (comp abs -) p1 p2)))
...
(defn p1 [in y]
  (let [x-coords (mapcat (fn [{[x _] :pos dist :dist}] [(+ x dist) (- x dist)]) in)]
    (c/count-matching (fn [x] (and (detected? in [x y]) (not-any? #(= [x y] (:beacon %)) in)))
                      (range (reduce min x-coords) (inc (reduce max x-coords))))))
